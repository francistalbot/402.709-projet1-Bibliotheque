...
@section('content')
    ...
    <form class="" action="{{ route('livres.index') }}" method="GET" style="min-width: 500px;">
        ...
        <div class="mb-3 row ">
            <div class="col grow-1 d-flex justify-content-start">
                <div class="d-flex align-items-center gap-2 w-50 " action="{{ route('livres.index') }}" method="GET"
            ...
            <div class="card mb-4 p-0 border-0">
                ...
                <div id="advancedSearch"
                    class="collapse {{ request()->hasAny(['search', 'author', 'annee_min', 'annee_max', 'prix_min', 'prix_max', 'category']) ? 'show' : '' }}">
                                    ...
                                    <select class="form-select" id="author" name="author">
                                        ...
                                        @foreach ($authors as $author)
                                            <option value="{{ $author->name }}"
                                               {{ $selectedAuthor == $author->name ? 'selected' : ''  }} >{{ $author->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <div class="row">
                                        <div class="col">
                                            <input type="number" class="form-control" name="annee_min"
                                                value="{{ request('annee_min') }}" placeholder="De" min="1500"
                                                max="{{ date('Y') }}">
                                        </div>
                                        <div class="col-auto align-self-center">à</div>
                                        <div class="col">
                                            <input type="number" class="form-control" name="annee_max"
                                                value="{{ request('annee_max') }}" placeholder="À" min="1500"
                                                max="{{ date('Y') }}">
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="row">
                                        <div class="col">
                                            <input type="number" class="form-control" name="prix_min"
                                                value="{{ request('prix_min') }}" placeholder="Prix min" min="0"
                                                step="0.01">
                                        </div>
                                        <div class="col-auto align-self-center">à</div>
                                        <div class="col">
                                            <input type="number" class="form-control" name="prix_max"
                                                value="{{ request('prix_max') }}" placeholder="Prix max" min="0"
                                                step="0.01">
                                        </div>
                                    </div>
                                </div>
    ...
    <h3>Livres en Promotion</h3>
    <div style="border: 1px solid rgb(214, 214, 214)"
        class="row gx-3 gx-lg-5 row-cols-1 row-cols-md-2 row-cols-xl-3 justify-content-left">


        @foreach ($livresPromo as $livre)
            <div class="col mb-5 cursor-pointer" href="{{ route('livres.show', ['livre' => $livre->id]) }}">
                <div class="card ">
                    <div class="card-header bg-info bg-opacity-10">
                        <h5 class="card-title">
                            <a class="card-link " href="{{ route('livres.show', ['livre' => $livre->id]) }}">
                                {{ $livre->titre }}</a>
                        </h5>
                    </div>
                    <div class="card-body position-relative">
                        @if ($livre->created_at >= now()->subDays(10))
                            <span class="badge bg-warning position-absolute"
                                style="top: 10px; right: 10px;">Nouveauté</span>
                        @endif
                        <div class="row mb-3">
                            <div class="col ">
                                <div>Auteur: <a class="card-link"
                                        href="{{ route('authors.show', ['author' => $livre->auteur->id]) }}">{{ $livre->auteur->name }}</a>
                                </div>
                                <div>Catégorie: <a class="card-link "
                                        href="{{ route('categories.show', ['category' => $livre->categorie->id]) }}">{{ $livre->categorie->name }}</a>
                                </div>
                                <div>Année de publication: {{ $livre->publication_annee }}</div>
                                <div>Prix: {{ $livre->prix }} $</div>
                            </div>
                        </div>
                        <a class="btn btn-outline-primary" href="{{ route('add.cart', $livre->id) }}">Ajouter au
                            panier</a>
